...
\ifnum \c@tocdepth >\m@ne
  ...
  \begingroup
    \parindent \z@ \rightskip \@pnumwidth
    \leavevmode\colorbox{bgalt}{%
      \vtop{%
        \hsize=\dimexpr\textwidth\relax
        \leavevmode\strut\color{fg}\bfseries\sffamily\uppercase{#1}%
        \nobreak\hfill\nobreak\hb@xt@\@pnumwidth{\hss #2}\strut
        \hangafter=1 % Apply hanging indent after the first line
        \hangindent=4cm % Indent lines starting from the second line
      }%
    }\par
    \penalty\@highpenalty
  \endgroup
...
\ifnum \c@tocdepth >\z@
  ...
  \begingroup
    \parindent \z@ \rightskip \@pnumwidth
    \leavevmode\colorbox{bg}{%
      \vtop{%
        \hsize=\dimexpr\textwidth-\@pnumwidth-4em\relax % Increase right padding for sections
